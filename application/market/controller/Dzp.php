...
class Dzp extends Controller
{
    ...
    public function index()
    {
        ...
        $res1 = Db::table('store_members')->field('id')
            ->where($where)
            ->find();
    ...
    public function save()
    {
        ...
        //查询会员信息
        $where1 = ['id'=>$id];
        $nick_u = Db::table('store_members')->field('nickname,mem_id')->where($where1)->find();
        $mem_id = $nick_u['mem_id'];

        //查询奖品信息
        $where2 = ['jp_id'=>$jiangpin];
        $jp_u = Db::table('store_dzp_jiangpin')->field('jp_name')->where($where2)->find();
        ...
        $dat = [
            ...
            $where = ['id'=>$id];
            $res = Db::table('store_members')->where($where)->update($data);

            Db::table('store_dzp_user_jiangpin')->insert($dat);
