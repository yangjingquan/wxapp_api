...
class Bis extends Controller{
    ...
    public function subJifenOrg(){
        ...
        try{
            //查询该订单产生的积分
            $order_res = Db::table('store_main_orders')->alias('main')
                ->where('main.id='.$order_id)
                ->find();
            ...
            //更新会员积分
            $mem_where = "mem_id = '$openid' and status = 1";
            $new_mem_res = Db::table('store_members')->where($mem_where)->setDec('jifen',$jifen);
            ...
            if($supplyProExist){
                ...
                if($checkAllSupplyPro['isAllSupplyPro']){
                    $res = $this->updateMainOrder($order_res['order_no'],$order_res['total_amount'],1);
                }else{
                    //如果不全是供货商品,把供货商品找出来，重新生成一条主订单作为供货订单，订单号为当前主订单号+'_1'后缀，
                    //并绑定供货订单与副订单表的供货商品所在记录
                    $subIdsArr = $checkAllSupplyPro['subIdsArr'];
                    //创建一条新的主订单,并更新副订单表
                    $newOrderNo = $this->createMainOrder($order_res,$subIdsArr);
                    //更新新旧主订单信息
                    $oldOrderRes = $this->updateMainOrder($order_res['order_no'],$order_res['total_amount']);
                    $newOrderRes = $this->updateMainOrder($newOrderNo,$order_res['total_amount'],1);
                }
            }else{
                //直接更新主订单
                $res = $this->updateMainOrder($order_res['order_no'],$order_res['total_amount']);
            }
    ...
    //更新主订单表
    public function updateMainOrder($orderNo,$totalAmount,$isSupplyOrder = 0){
        //更改外部订单状态,记录流水号
        $mainOrderData = [
            'order_status' => 3,
