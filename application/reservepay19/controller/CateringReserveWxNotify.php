...
class CateringReserveWxNotify extends \WxPayNotify{
    ...
    public function NotifyProcess($data, &$msg){
        if($data['result_code'] == 'SUCCESS'){
            ...
            try{
                //通过订单号查询外部订单信息(订单总金额)
                $out_order_info = Db::table('cy_pre_orders')->field('openid,bis_id,deposit')->where('order_no = '.$orderNo)->find();
                $openid = $out_order_info['openid'];
                $bisId = $out_order_info['bis_id'];
                //添加积分
                $this->addJifen($bisId,$openid,$out_order_info['deposit'],$orderNo);
