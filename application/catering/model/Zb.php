...
class Zb extends Model
{
    ...
    public function getUserinfo($param)
    {
       ...
       $mem_id = !empty($param['mem_id']) ? $param['mem_id'] : '';
        $res = Db::table('cy_members')->where("mem_id = '$mem_id' and status = 1")->find();
        if (!$res) {
            echo json_encode([
                'statuscode' => 0,
                'message' => '获取失败'
    ...
    public function editUserinfo($param)
    {
        ...
        $re = Db::table('cy_members')->where("mem_id = '$mem_id' and status = 1")->find();
        $data = [
            'truename'=>$truename,
            'mobile'=>$mobile,
            'address'=>$address,
        ...
        $res = Db::table('cy_members')->where("mem_id = '$mem_id' and status = 1")->update($data);
