...
class Address extends Model{
    ...
    public function getAddressInfo($openid){
        $res = DB::table('cy_address')->field('id as a_id,rec_name,mobile,province,city,area,address')->where("mem_id = '$openid' and status = 1 ")->order('create_time desc')->select();
    ...
    public function addAddress($param){
        ...
        $res = DB::table('cy_address')->insert($data);
    ...
    public function getAddressInfoById($a_id){
        $res = DB::table('cy_address')->field('id as a_id,rec_name,mobile,province,city,area,address')->where("id = $a_id")->find();
    ...
    public function updateAddress($param){
        ...
        $res = DB::table('cy_address')->where("id = $aid")->update($data);
        return $res;
    }
    ...
    public function chooseAddress($param){
        ...
        $data1['is_default'] = 0;
        $default_res = Db::table('cy_address')->where($where)->update($data1);

        //设置选中的地址is_default为1
        $con = "id = $selected_id";
        $data2['is_default'] = 1;
        $res = Db::table('cy_address')->where($con)->update($data2);
